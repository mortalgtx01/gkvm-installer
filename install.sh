...
NODE_MAJOR=20
# IMPORTANT: This is a distribution artifact URL, not a GitHub credential.
# Put a URL to the package served by your artifact server here.
ARTIFACT_URL="${ARTIFACT_URL:-https://YOUR-DOWNLOAD-HOST/download/gkvm-panel.tar.gz}"
ARTIFACT_KEY="${ARTIFACT_KEY:-CHANGE_ME}"
...
[[ "$ARTIFACT_URL" != *'YOUR-DOWNLOAD-HOST'* ]] || die 'Set ARTIFACT_URL in install.sh before publishing.'
[[ "$ARTIFACT_KEY" != 'CHANGE_ME' ]] || die 'Set ARTIFACT_KEY in install.sh before publishing.'
...
if command -v apt-get >/dev/null 2>&1; then
  ...
  apt-get install -y curl ca-certificates gnupg build-essential python3 sqlite3 libsqlite3-dev \
    ...
    net-tools unzip rsync lsof procps iproute2 sudo
elif command -v dnf >/dev/null 2>&1; then
  dnf -y install curl ca-certificates gcc gcc-c++ make python3 sqlite sqlite-devel \
    qemu-system-x86-core qemu-img genisoimage xorriso bridge-utils net-tools unzip rsync lsof procps-ng iproute sudo
elif command -v yum >/dev/null 2>&1; then
  yum -y install curl ca-certificates gcc gcc-c++ make python3 sqlite sqlite-devel \
    qemu-img genisoimage xorriso bridge-utils net-tools unzip rsync lsof procps-ng iproute sudo
else
...
if ss -ltn 2>/dev/null | grep -qE ":${PANEL_PORT}[[:space:]]"; then
...
curl --fail --silent --show-error --location --retry 3 --connect-timeout 10 \
  ...
  "$ARTIFACT_URL" -o "$ARCHIVE"
[[ -s "$ARCHIVE" ]] || die 'Artifact download was empty.'
ok 'Panel artifact downloaded.'

info 'Validating artifact...'
tar -tzf "$ARCHIVE" >/dev/null || die 'Downloaded artifact is not a valid gzip tar archive.'
rm -rf "$TMP_DIR/panel"
...
[[ -f "$SOURCE_DIR/package.json" ]] || die 'Artifact does not contain package.json.'
[[ -f "$SOURCE_DIR/package-lock.json" ]] || die 'Artifact does not contain package-lock.json.'
[[ -f "$SOURCE_DIR/app.js" ]] || die 'Artifact does not contain app.js.'
...
EOF_ENV
  printf '%s\n' "$ADMIN_PASSWORD" > "$TMP_DIR/admin-password"
...
if [[ ! -f "$INSTALL_DIR/.gkvm-admin-password-shown" ]]; then
  # Read the generated password only if we created it during this run.
  if [[ -f "$TMP_DIR/admin-password" ]]; then
    ...
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/.gkvm-admin-password-shown"
    chmod 600 "$INSTALL_DIR/.gkvm-admin-password-shown"
  fi
fi
line
